...
const Header: React.FC<{section?: string}> = ({section = '기억 장면'}) => (
  <>
    <div style={{position:'absolute',top:HEADER_TOP,left:SIDE,right:RIGHT_UI,display:'flex',alignItems:'center',gap:18,zIndex:20,fontFamily:FONT}}>
    ...
    <div style={{position:'absolute',top:118,left:SIDE,zIndex:20,fontFamily:FONT,fontSize:22,fontWeight:800,color:MUTED}}>{section}</div>
  </>
);
...
const ChoiceList: React.FC<{highlight?:number}> = ({highlight}) => (
  <div style={{marginTop:40,display:'grid',gap:14,width:'100%'}}>
    {current.choices.map((text,index) => {
      ...
      return (
        <div key={`${index}-${text}`} style={{border:`2px solid ${selected ? ORANGE : '#555A60'}`,background:selected ? ORANGE : 'rgba(255,255,255,.015)',borderRadius:16,padding:'16px 22px',display:'grid',gridTemplateColumns:'48px 1fr',alignItems:'center',fontSize:25,lineHeight:1.4,fontWeight:800,textAlign:'left',boxShadow:selected ? '0 0 24px rgba(255,138,0,.18)' : 'none'}}>
  ...
  return (
    <Stage section="문제">
      <div style={{position:'relative',width:'100%',paddingTop:active ? 112 : 0}}>
        ...
        <div style={{fontSize:48,lineHeight:1.34,fontWeight:900,letterSpacing:-2.1,padding:'0 16px'}}><span style={{color:ORANGE}}>Q. </span>{current.question}</div>
        <ChoiceList />
...
const Explanation = () => (
  <Stage section="핵심 해설">
    ...
    <div style={{marginTop:46,fontSize:40,lineHeight:1.62,fontWeight:800,letterSpacing:-1.2}}>{current.explanation}</div>
    {current.examLink ? <div style={{marginTop:42,fontSize:27,lineHeight:1.55,color:'#E6E6E2',fontWeight:700}}>{current.examLink}</div> : null}
  ...
  <Stage section="오답 함정">
    ...
    <div style={{marginTop:42,display:'grid',gridTemplateColumns:'1fr 1fr',gap:22,width:'100%'}}>
      {(current.wrongTraps || []).map(([title,description]) => (
        <div key={title} style={{minHeight:190,padding:'26px 24px',border:'1px solid #45494D',borderRadius:18,background:'rgba(255,255,255,.025)',display:'flex',flexDirection:'column',justifyContent:'center'}}>
...
const MemoryImage = () => (
  <AbsoluteFill style={{background:BG,color:WHITE,fontFamily:FONT,overflow:'hidden'}}>
    ...
    <div style={{position:'absolute',left:SIDE,right:RIGHT_UI,bottom:285,zIndex:10,textAlign:'center'}}>
      <div style={{maxWidth:760,margin:'0 auto'}}>
        ...
        <div style={{marginTop:14,fontSize:43,lineHeight:1.35,fontWeight:900}}>{current.memoryHeadline || current.memoryTip || current.title}</div>
      </div>
    </div>
  </AbsoluteFill>
);
...
const Mnemonic = () => (
  <Stage section="기억법">
    ...
    <div style={{marginTop:50,fontSize:46,lineHeight:1.58,fontWeight:900}}>{current.mnemonicParts?.length ? renderParts(current.mnemonicParts) : current.memoryTip}</div>
    {current.mnemonicSublineParts?.length ? <div style={{marginTop:38,fontSize:29,lineHeight:1.62,color:'#E2E2DE',fontWeight:800}}>{renderParts(current.mnemonicSublineParts)}</div> : null}
  </Stage>
);
...
const Recap = () => (
  <Stage section="오늘의 정리">
    ...
    <div style={{marginTop:48,border:'2px solid #665A49',borderRadius:22,padding:'42px 36px',fontSize:43,lineHeight:1.55,fontWeight:900}}>
    ...
    {current.recapDetail ? <div style={{marginTop:38,fontSize:29,lineHeight:1.6,fontWeight:700}}>{current.recapDetail}</div> : null}
  </Stage>
);

const CTA = () => (
  <Stage section="다음 문제">
    <div style={{fontSize:50,lineHeight:1.55,fontWeight:900}}>역사 공부,<br />매일 <span style={{color:ORANGE}}>1분</span>이면 충분!</div>
    <div style={{margin:'50px auto 0',width:'82%',borderTop:'2px solid #6B655F',paddingTop:30,fontSize:27,fontWeight:800}}>좋아요　 댓글　 저장</div>
    <div style={{marginTop:60,fontSize:32,lineHeight:1.55,fontWeight:800}}>다음 기억 장면에서<br />또 만나요!</div>
  </Stage>
);
